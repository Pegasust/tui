...
var (
	...
	cursorLeft      = key.NewBinding(key.WithKeys("h", "left"), key.WithHelp("←", "½ back"))
	cursorRight     = key.NewBinding(key.WithKeys("l", "right"), key.WithHelp("→", "½ forward"))
	pageUp          = key.NewBinding(key.WithKeys("u", "ctrl+u", "pgup"), key.WithHelp("pgup", "1 back"))
	pageDown        = key.NewBinding(key.WithKeys("d", "ctrl+d", "pgdown", spacebar), key.WithHelp("pgdn", "1 forward"))
	...
	showReadme      = key.NewBinding(key.WithKeys("K", "?"), key.WithHelp("?", "inspect"))
